        ...
        <form method="post" enctype="application/x-www-form-urlencoded" class="needs-validation was-validated " novalidate>
            <div class="form-group">
                ...
                <input type="text" class="form-control box" id="name" required name="name"
                       value="<?= isset($user) ? $user->getUsername() : '' ?>">
            ...
            <div class="form-group select_box">
                <select name="work_in" id="work_in" class="custom-select" required>
                    <option value="" disabled > <?= isset($Text_work_in) ? $Text_work_in : '' ?> </option>
                    <?php
                        if (isset($places) && !empty($places)){
                            foreach ($places as $place ){
                    ?>
                        <option value="<?= $place->getPlaceId()?>" <?= isset($user) && $user->getWorkIn() == $place->getPlaceId() ? 'selected' : ''  ?> > <?= $place->getPlaceName() ?> </option>
                    <?php
                            }
                        }
                    ?>
            ...
            <div class="form-group select_box" >
                <select name="group" id="group" class="custom-select" required>
                    <option value="0" <?= isset($user) && $user->getGroupId() == '0' ? 'selected' : ''  ?>  > <?= isset($Text_member) ? $Text_member : '' ?> </option>
                    <option value="1" <?= isset($user) && $user->getGroupId() == '1' ? 'selected' : ''  ?>> <?= isset($Text_admin) ? $Text_admin : '' ?> </option>
                </select>
                <div class="valid-feedback"><?= isset($valid_msg) ? $valid_msg : '' ?> </div>
